...
func (pr *PageCollection) ShowMultipleChoicePage(mc *models.MultipleChoice) g.Node {
	...
	return Div(Class("flex flex-col min-h-[calc(100dvh-10rem)] space-y-4"),
	...
	return g.Group(
		[]g.Node{
			H3(
				...
				Span(Class("font-bold text-sky-700 font-mono text-2xl tracking-widest pt-1 px-1 rounded bg-slate-300"),
					g.Text(codeStr[0:4]),
					g.Text(" "),
					g.Text(codeStr[4:8]),
				),
